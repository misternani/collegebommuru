...
if (in_array($fileType1, $allowTypes) ) {
    ...
    if (move_uploaded_file($_FILES["reviewprofile"]["tmp_name"], $targetFilePath1)) {
        ...
        $query = "INSERT INTO polycet (imagepath) 
                  VALUES ('$fileName1')";
        ...
        if ($result) {
            ...
            echo 'Polycet Information Added successfully.';
            echo '<br>'; // Add line break for button positioning

            // Button HTML
            echo '<a href="addpolyinfo.php" style="display: inline-block; background-color: #1a73e8; padding: 5px 10px; border-radius: 8px; color: white; text-decoration: none;font-size: 38px;">Go Back</a>';
