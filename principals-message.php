...
<!-- .navbar -->
  ...
  <main id="main">
    <br>
        <h1 style="display: flex;justify-content: center;align-items: center;margin-top: 100px;font-weight: bold;">Principal's Message</h1>
...
<div class="card card-body shadow p-4 align-items-start mt-3"  style="border: 0px solid rgba(255, 255, 255, 0);">
    <div class="page-view">
       <p>
       <?php
             $q = mysqli_query($con, "SELECT `path` FROM special_images WHERE role='principal' ") or die('Error197');
         while ($row = mysqli_fetch_array($q)) {
          echo '<img src="sp_img/'.$row[0].'" style="display: flex;justify-content: center;align-items: center;width: 300px;height: 340px;border-radius: 15px;margin-left: auto;margin-right: auto;">
        <br>';
         }?>
         <?php
             $q = mysqli_query($con, "SELECT p_message FROM textdata") or die('Error197');
         while ($row = mysqli_fetch_array($q)) {
          echo $row[0];
         }?>
    </p>
